...
public class Signin extends CommonData {
    ...
    public String[] start(Context ctx, boolean isAutoSign, String cfg, String user, String pwd) {
        ...
        try{
            ...
            res = Jsoup.connect(captchaHashUrl).header("X-Requested-With", "XMLHttpRequest").cookies(cookies).userAgent(UA_CHROME).timeout(TIME_OUT).referrer(ajaxLoginUrl).ignoreContentType(true).method(Method.GET).execute();
            ...
            res = Jsoup.connect(ajaxLoginUrl).header("X-Requested-With", "XMLHttpRequest").data(postDatas).cookies(cookies).userAgent(UA_CHROME).timeout(TIME_OUT).referrer(ajaxLoginUrl).ignoreContentType(true).method(Method.POST).execute();
            cookies.putAll(res.cookies());
